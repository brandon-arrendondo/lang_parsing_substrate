...
| `src/fingerprint.rs` | AST subtree structural fingerprinting + corpus-wide clone grouping (Tier 5): `function_fingerprints()`/`block_fingerprints()` (two `FingerprintTier`s — whole functions vs. loop/conditional/switch-shaped subtrees), `structural_hash()` for an arbitrary caller-supplied node, `duplicate_groups()` (groups by `(hash, tier)`); see `DETECT_FINE_GRAINED_DUPLICATES.md` for the design handoff |
