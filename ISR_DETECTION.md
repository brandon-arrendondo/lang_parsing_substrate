...
aurora-lint-side judgment call.

Originally written up from aurora-lint's side (task 608, gated on task 607,
both follow-ons of task 151 —
`aurora-lint/docs/design/concurrency-rule-evaluation.md` is the source design
doc) so an agent here could scope and design it without re-deriving the
background from aurora-lint. Smaller and more self-contained than
...
## Why aurora-lint needs this — and why it isn't aurora-lint-specific

aurora-lint's CON03-C/CON07-C/CON33-C (CERT-C concurrency rules) currently fire
...
- `aurora-lint/src/rules/cert_c/CONC/CON03-C/con03_c.rs` — flags every
...
- `aurora-lint/src/rules/cert_c/CONC/CON07-C/con07_c.rs` — same gap; only
  checks for a mutex/atomic type or a lock/unlock call nearby.
- `aurora-lint/src/rules/cert_c/CONC/CON33-C/con33_c.rs` — fires on a fixed
...
**What should stay aurora-lint-side, deliberately not asked for here:**
...
## Evidence this pays off — reproducible, from aurora-lint's own data

aurora-lint's task 607 built a crude stopgap classifier
(`aurora-lint/bench/concurrency_context.py`) — a whole-file regex for
...
call-graph reachability (aurora-lint-side follow-on, task 608), is expected to
...
crate's or aurora-lint's checked-out benchmark projects; reported second-hand
via aurora-lint's task 151, not independently reproducible from either repo)
...
- **`src/calls.rs`** — `CallEdge { caller, callee, is_external }`,
  ...
  exactly the per-file call-graph primitive aurora-lint's reachability
...
problem aurora-lint's own EXP33-C GNU-asm fix dealt with post tree-sitter
upgrade — see `docs/migration-aurora-lint.md`) or textual pattern matching
...
decide whether that's still fine (aurora-lint's reachability analysis roots at
...
non-issue depending on how aurora-lint's reachability analysis is structured.
...
- **Which macro names count as "ISR-defining"?** `ISR` (AVR-libc) is the one
  concretely cited in aurora-lint's evidence (Catapult, AVR-style embedded).
  ...
  and let the caller (aurora-lint) filter by name — this fits the crate's
...
- **Is a low-confidence name-substring fallback tier worth including at
  ...
  actively misleading? Leaning no — aurora-lint already has its own crude
...
- **IAR `#pragma vector=`/`__interrupt`, Keil `__irq`** — in scope for v1, or
  ...
  forever, aurora-lint's review process delta-adjudicates before claiming a
...
- **`collect_functions`'s macro-function exclusion** (see "Cross-cutting
  ...
  whether aurora-lint's downstream reachability analysis can see calls made
